...
static void if_tag_stat_update(const char *ifname, kuid_t uid,
			       ...
			       int proto, int bytes)
{
	...
	MT_DEBUG("qtaguid: if_tag_stat_update(ifname=%s "
		"uid=%u sk=%p dir=%d proto=%d bytes=%d)\n",
		 ifname, from_kuid(&init_user_ns, uid), sk, direction, proto, bytes);
...
static void account_for_uid(const struct sk_buff *skb,
			    const struct sock *alternate_sk, kuid_t uid,
			    struct xt_action_param *par)
{
	const struct net_device *el_dev;
...
static bool qtaguid_mt(const struct sk_buff *skb, struct xt_action_param *par)
{
	...
	if (sk == NULL || sk->sk_socket == NULL) {
		...
		if (!(info->match & XT_QTAGUID_UID))
			account_for_uid(skb, sk, make_kuid(&init_user_ns, 0), par);
...
static int ctrl_cmd_delete(const char *input)
{
	...
	argc = sscanf(input, "%c %llu %u", &cmd, &acct_tag, &temp_uid);
	uid = make_kuid(&init_user_ns, temp_uid);
	...
	while (node) {
		st_entry = rb_entry(node, struct sock_tag, sock_node);
		entry_uid = make_kuid(&init_user_ns, get_uid_from_tag(st_entry->tag));
		node = rb_next(node);
		if (!uid_eq(entry_uid, uid))
			continue;

		CT_DEBUG("qtaguid: ctrl_delete(%s): st tag=0x%llx (uid=%u)\n",
			 input, st_entry->tag, from_kuid(&init_user_ns, entry_uid));
	...
	list_for_each_entry(iface_entry, &iface_stat_list, list) {
		...
		while (node) {
			...
			entry_uid = make_kuid(&init_user_ns, get_uid_from_tag(ts_entry->tn.tag));
			node = rb_next(node);

			CT_DEBUG("qtaguid: ctrl_delete(%s): "
				 "ts tag=0x%llx (uid=%u)\n",
				 input, ts_entry->tn.tag, from_kuid(&init_user_ns, entry_uid));
			...
			if (!acct_tag || ts_entry->tn.tag == tag) {
				CT_DEBUG("qtaguid: ctrl_delete(%s): "
					 ...
					 from_kuid(&init_user_ns, entry_uid));
	...
	while (node) {
		...
		entry_uid = make_kuid(&init_user_ns, utd_entry->uid);
		node = rb_next(node);

		CT_DEBUG("qtaguid: ctrl_delete(%s): "
			 "utd uid=%u\n",
			 input, from_kuid(&init_user_ns, entry_uid));
...
static int ctrl_cmd_counter_set(const char *input)
{
	...
	argc = sscanf(input, "%c %d %u", &cmd, &counter_set, &temp_uid);
	uid = make_kuid(&init_user_ns, temp_uid);
	CT_DEBUG("qtaguid: ctrl_counterset(%s): argc=%d cmd=%c "
		 "set=%d uid=%u\n", input, argc, cmd,
		 counter_set, temp_uid);
	if (argc != 3) {
		res = -EINVAL;
		goto err;
	}
...
static int ctrl_cmd_tag(const char *input)
{
	...
	argc = sscanf(input, "%c %d %llu %u", &cmd, &sock_fd, &acct_tag, &temp_uid);
	uid = make_kuid(&init_user_ns, temp_uid);
